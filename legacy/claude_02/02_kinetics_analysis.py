...
from enzyme_kinetics import EnzymeKineticsAnalyzer, CalibrationCurve, KineticsPlotter
...
def example_kinetics_analysis():
    ...
    print(
        df_results_sorted[
            ["peak_id", "Km_MM", "Vmax_MM", "R2_MM", "kcat_over_km"]
        ].to_string()
    )
    print()
...
def example_single_peak_analysis():
    ...
    # Analyze
    analyzer = EnzymeKineticsAnalyzer(
        enzyme_concentration_um=12.25, reaction_time_seconds=10800
    )

    result = analyzer.analyze_peak(
        "HTAL_example", substrate_conc, velocity, velocity_std=velocity_std
    )

    print(f"Peak ID: {result.peak_id}")
    print("Michaelis-Menten Fit:")
    ...
    if result.kcat is not None:
        ...
        print(
            f"  kcat/Km = {result.kcat_over_km:.4f} ± {result.kcat_over_km_std:.4f} sec⁻¹"
        )
