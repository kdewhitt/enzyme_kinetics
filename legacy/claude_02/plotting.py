...
from analyzer import EnzymeKineticsResult
...
class KineticsPlotter:
    ...
    def plot_michaelis_menten_curve(
        ...
        if show_fit:
            ...
            v_fit = result.mm_params.Vmax * S_smooth / (result.mm_params.Km + S_smooth)
    ...
    def plot_lineweaver_burk(
        ...
        if result.lb_params is None:
            ax.text(
                0.5,
                0.5,
                "Lineweaver-Burk fit not available",
                ha="center",
                va="center",
    ...
    @staticmethod
    def plot_residuals(
        result: EnzymeKineticsResult, ax: plt.Axes | None = None
    ) -> plt.Axes:
    ...
    def plot_comparison_panel(
        results: list[EnzymeKineticsResult], output_path: Path | str | None = None
    ) -> None:
    ...
    def plot_efficiency_comparison(
        ...
        valid_results = [
            r for r in results if r.mm_params.Km > 0 and r.kcat_over_km is not None
        ]
        ...
        valid_results = sorted(
            valid_results, key=lambda r: r.kcat_over_km, reverse=True
        )
        ...
        ax.bar(
            peaks,
            [r.mm_params.Km for r in valid_results],
            color=colors,
            alpha=0.7,
            edgecolor="black",
            linewidth=2,
        ...
        ax.bar(
            peaks,
            [r.mm_params.Vmax for r in valid_results],
            color=colors,
            alpha=0.7,
            edgecolor="black",
            linewidth=2,
        ...
        ax.bar(
            peaks,
            [r.kcat for r in valid_results if r.kcat is not None],
            color=colors,
            alpha=0.7,
            edgecolor="black",
            linewidth=2,
        ...
        ax.bar(
            peaks,
            [r.kcat_over_km for r in valid_results],
            color=colors,
            alpha=0.7,
            edgecolor="black",
            linewidth=2,
