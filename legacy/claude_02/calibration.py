...
class CalibrationCurve:
    ...
    def fit(
        ...
        popt, pcov = curve_fit(
            ...
            concentrations,
            peak_areas,
            sigma=peak_areas_std
            if peak_areas_std is not None and np.any(peak_areas_std > 0)
            else None,
            absolute_sigma=True,
        )
    ...
    def area_to_concentration(
        self, peak_area: np.ndarray | float
    ) -> np.ndarray | float:
    ...
    def area_to_concentration_with_error(
        self, peak_area: float, peak_area_std: float
    ) -> tuple[float, float]:
        ...
        # Total uncertainty
        conc_std = np.sqrt(
            (dC_darea * numerator_error) ** 2 + (dC_dslope * self.params.slope_std) ** 2
        )
    ...
    def is_valid(self) -> bool:
        ...
        return (
            self.params is not None and self.params.r2 > 0.95 and self.params.slope > 0
        )
    ...
    def save(self, filepath: str | Path) -> None:
        ...
        with open(filepath, "w") as f:
            ...
            f.write("PARAMETERS:\n")
            f.write(
                f"  Slope:        {self.params.slope:.10f} ± {self.params.slope_std:.10f} area/µM\n"
            )
            f.write(
                f"  Intercept:    {self.params.intercept:.10f} ± {self.params.intercept_std:.10f} area\n"
            )
            f.write(f"  R²:           {self.params.r2:.10f}\n")
            f.write(f"  RMSE:         {self.params.rmse:.6f} area\n")
            f.write(f"  N points:     {self.params.n_points}\n")
            f.write(
                f"  Conc range:   {self.params.conc_range[0]:.2f}–{self.params.conc_range[1]:.2f} µM\n\n"
            )

            f.write("INVERSE FUNCTION:\n")
            f.write(
                f"  [concentration] = (peak_area - {self.params.intercept:.10f}) / {self.params.slope:.10f}\n\n"
            )
    ...
    def plot(self, output_path: str | Path | None = None) -> None:
        ...
        ax.set_title(
            f"Calibration Curve (R² = {self.params.r2:.6f})",
            fontsize=13,
            fontweight="bold",
        )
        ...
        ax.errorbar(
            conc,
            residuals,
            yerr=area_std if area_std is not None else None,
            fmt="o",
            markersize=10,
            capsize=6,
            capthick=2.5,
        ...
        ax.axhline(y=0, color="black", linestyle="-", linewidth=2, zorder=2)
        ax.axhline(
            y=3, color="red", linestyle="--", linewidth=2, alpha=0.5, label="±3σ"
        )
