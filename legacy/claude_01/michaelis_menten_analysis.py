...
def analyze_enzyme_kinetics(data_path, output_dir="./"):
    ...
    for peak_id, group_data in df_hexcoa.groupby("peak_id"):
        ...
        try:
            ...
            print(f"\n✓ {peak_id}")
            print(
                f"  Non-linear MM: Vmax={Vmax_mm:.2f}±{perr_mm[0]:.2f}, "
                f"Km={Km_mm:.4f}±{perr_mm[1]:.4f}, R²={r2_mm:.4f}"
            )
            print(
                f"  Lineweaver-Burk: Vmax={Vmax_lb:.2f}, Km={Km_lb:.4f}, R²={r_value_lb**2:.4f}"
            )
...
def create_plots(df, results_df, output_dir):
    ...
    for idx, (_, result) in enumerate(results_df.iterrows()):
        ...
        ax_mm.axvline(
            x=result["Km_MM"], color="gray", linestyle="--", alpha=0.5, linewidth=1
        )
        ...
        v_inv_fit = lineweaver_burk_line(
            S_inv_smooth, result["Vmax_MM"], result["Km_MM"]
        )
        ...
        ax_lb.plot(
            S_inv_smooth, v_inv_fit, "-", linewidth=2.5, color="darkred", label="Fit"
        )
    ...
    fig_mm.savefig(
        f"{output_dir}/michaelis_menten_curves.png", dpi=300, bbox_inches="tight"
    )
    fig_lb.savefig(
        f"{output_dir}/lineweaver_burk_plots.png", dpi=300, bbox_inches="tight"
    )

    print("\n✓ Plots saved:")
    print("  - michaelis_menten_curves.png")
    print("  - lineweaver_burk_plots.png")
