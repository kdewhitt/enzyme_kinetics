...
def _kcat_km_with_covariance(
    ...
    Args:
        vmax: Fitted Vmax (or Vmax in calibrated units).
        km: Fitted Km (or k_half for Hill).
        enzyme_conc_um: Total enzyme concentration in µM.
        pcov: Full 2×2 (or larger) covariance matrix from curve_fit.

    Returns:
        Tuple of (kcat_km, kcat_km_se). Returns (nan, nan) if km ≤ 0 or
        pcov contains nan (e.g. for LB fits).
    """
    if km <= 0 or vmax <= 0:
    ...
    kcat_km = vmax / (enzyme_conc_um * km)

    # Partial derivatives of f = vmax / (E * km)
    df_dvmax = 1.0 / (enzyme_conc_um * km)
    df_dkm = -vmax / (enzyme_conc_um * km ** 2)
    ...
    kcat_km_se = float(np.sqrt(max(var_kcat_km, 0.0)))  # clamp numerical negatives
    return float(kcat_km), kcat_km_se
...
class KineticConstants:
    ...
    kcat_km: float
    kcat_km_se: float
    ki: float | None = None          # FIX 4: typed Ki at KineticConstants level
    ki_se: float | None = None       # FIX 4: typed Ki_se at KineticConstants level
...
def derive_constants(
    ...
    kcat/Km uncertainty is propagated via the full Vmax–Km covariance matrix
    (see _kcat_km_with_covariance), correctly accounting for the anti-correlation
    between Vmax and Km that arises from MM-family non-linear fitting.
    ...
    Args:
        ...
        fit: FitResult from any kinetic model fit.
        enzyme_conc_um: Total enzyme concentration in µM.
    ...
    # FIX 1: use full covariance propagation instead of relative-error approximation
    kcat_km, kcat_km_se = _kcat_km_with_covariance(
