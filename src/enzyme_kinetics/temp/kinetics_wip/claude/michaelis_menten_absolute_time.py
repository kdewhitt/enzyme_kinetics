...
def analyze_with_time_units(data_path, output_dir='./'):
    ...
    print(f"\n{'=' * 90}")
    print(f"ENZYME KINETICS WITH ABSOLUTE TIME UNITS")
    print(f"{'=' * 90}")
    print(f"Enzyme Concentration:     {ENZYME_CONCENTRATION_UM} µM")
    print(f"Reaction Time:            {REACTION_TIME_HOURS} hours = {REACTION_TIME_SECONDS} seconds")
    print(f"Peak Area Units:          Relative (area per 3-hour reaction)")
    print(f"{'=' * 90}\n")
    ...
    for peak_id, group_data in df_hexcoa.groupby('peak_id'):
        ...
        try:
            ...
            popt_mm, pcov_mm = curve_fit(
                michaelis_menten, S, area, p0=p0, maxfev=5000,
            ...
            if Km > 0:
                ...
                # Error propagation
                rel_err_kcat = kcat_per_sec_std / kcat_per_sec if kcat_per_sec > 0 else 0
            ...
            print(f"    Vmax (area)     = {Vmax_area:8.1f} ± {perr_mm[0]:8.1f} area/(3h)")
            print(f"    Vmax (per hour) = {Vmax_per_hour:8.2f} area/hour")
            print(f"    Vmax (per sec)  = {Vmax_per_sec:8.6f} area/sec")
            print(f"    R²              = {r2:8.4f}")
            print(f"  ")
            print(f"  Enzyme-Normalized (kcat = Vmax / [E={ENZYME_CONCENTRATION_UM} µM]):")
            print(f"    kcat            = {kcat_per_sec:8.6f} ± {kcat_per_sec_std:8.6f} area·µM⁻¹·sec⁻¹")
            print(f"    kcat/Km         = {kcat_over_km:12.4f} ± {kcat_over_km_std:12.4f} sec⁻¹")

            if kcat_over_km > 0:
                print(f"    Efficiency      = {kcat_over_km:.2f} (reactions/sec relative to Km)")
            print(f"  ")
...
def create_time_unit_plots(results_df, output_dir):
    ...
    valid_results = valid_results.sort_values('kcat_over_km_per_sec_per_um', ascending=False)
    ...
    bars = ax.bar(peaks, valid_results['Km'].values, color=colors, alpha=0.7, edgecolor='black', linewidth=2)
    ...
    bars = ax.bar(peaks, valid_results['Vmax_per_sec'].values, color=colors, alpha=0.7, edgecolor='black', linewidth=2)
    ...
    for i, (peak, val) in enumerate(zip(peaks, valid_results['Vmax_per_sec'].values)):
        ax.text(i, val, f'{val:.2e}', ha='center', va='bottom', fontweight='bold', fontsize=8)

    # ===== Plot 3: kcat =====
    ax = fig.add_subplot(gs[0, 2])
    bars = ax.bar(peaks, valid_results['kcat_per_sec'].values, color=colors, alpha=0.7, edgecolor='black', linewidth=2)
    ...
    ax.grid(axis='y', alpha=0.3)
    for i, (peak, val) in enumerate(zip(peaks, valid_results['kcat_over_km_per_sec_per_um'].values)):
        ax.text(i, val, f'{val:.2e}', ha='center', va='bottom', fontweight='bold', fontsize=8)

    # ===== Plot 5: R² values =====
    ax = fig.add_subplot(gs[1, 1])
    bars = ax.bar(peaks, valid_results['R2'].values, color=colors, alpha=0.7, edgecolor='black', linewidth=2)
    ax.axhline(y=0.7, color='red', linestyle='--', linewidth=2, label='Good fit threshold')
    ...
    for i, (peak, val) in enumerate(zip(peaks, valid_results['R2'].values)):
        ax.text(i, val, f'{val:.3f}', ha='center', va='bottom', fontweight='bold', fontsize=9)
    ...
    width = 0.35
    ax.bar(
        x - width / 2, valid_results['Vmax_per_hour'].values, width, label='Per hour',
        color='steelblue', alpha=0.7, edgecolor='black', linewidth=1.5,
    )
    ax.bar(
        x + width / 2, valid_results['Vmax_per_sec'].values * 3600, width, label='Per sec (scaled)',
    ...
    summary_data = [
        ['Rank', 'Peak', 'Km (µM)', 'kcat (area·µM⁻¹·sec⁻¹)', 'kcat/Km (sec⁻¹)', 'Interpretation'],
    ]
    ...
    for rank, (_, row) in enumerate(valid_results.iterrows(), 1):
        summary_data.append(
            [
                ...
                "Most efficient" if rank == 1 else "Very good" if rank == 2 else "Good" if rank == 3 else "Fair",
            ],
        )

    table = ax.table(
        cellText=summary_data, cellLoc='center', loc='center',
    ...
    ax.text(
        0.5, 1.1, 'Enzyme Kinetics Summary (Ranked by Catalytic Efficiency)',
        ha='center', fontsize=13, fontweight='bold', transform=ax.transAxes,
    )

    fig.suptitle(
        f'Michaelis-Menten Analysis with Absolute Time Units\n'
        f'[Enzyme] = {ENZYME_CONCENTRATION_UM} µM  |  Reaction Time = {REACTION_TIME_HOURS} hours = '
        f'{REACTION_TIME_SECONDS} seconds',
        fontsize=14, fontweight='bold', y=0.995,
    )

    fig.savefig(f'{output_dir}/enzyme_kinetics_time_units.png', dpi=300, bbox_inches='tight')
    print(f"✓ Comprehensive kinetics plot saved: enzyme_kinetics_time_units.png\n")
...
if __name__ == '__main__':
    ...
    valid = results[results['Km'] > 0].sort_values('kcat_over_km_per_sec_per_um', ascending=False)

    cols_to_show = [
        'peak_id', 'Km', 'Vmax_per_sec', 'kcat_per_sec', 'kcat_over_km_per_sec_per_um', 'R2',
    ]
