...
def generate_calibration_curve(csv_path, output_dir='./'):
    ...
    print(f"Linear Model: peak_area = slope × [CoA] + intercept\n")
    ...
    if np.any(outliers):
        print(f"⚠ Warning: {np.sum(outliers)} potential outliers detected (|residuals| > 3σ)")
        for idx in np.where(outliers)[0]:
            print(f"    Point {idx + 1}: [CoA]={coa_conc[idx]:.2f}, residual={residuals[idx]:.2f}")
    else:
        print(f"✓ No outliers detected (all residuals < 3σ)")
    ...
    ax = axes[0, 0]
    ax.errorbar(
        coa_conc, peak_area, yerr=peak_area_std, fmt='o', markersize=10,
        capsize=6, capthick=2.5, color='steelblue', elinewidth=2,
        markeredgewidth=2.5, markeredgecolor='navy', label='Measurements',
        zorder=3,
    )

    coa_smooth = np.linspace(
        coa_conc.min() - coa_conc.max() * 0.1,
        coa_conc.max() * 1.15, 200,
    )
    ...
    ci = 1.96 * se * np.sqrt(
        1 / len(coa_conc) + (coa_smooth - np.mean(coa_conc)) ** 2 / np.sum((coa_conc - np.mean(coa_conc)) ** 2),
    )
    ax.fill_between(coa_smooth, area_fit - ci, area_fit + ci, alpha=0.2, color='red', label='95% CI')
    ...
    ax = axes[0, 1]
    ax.errorbar(
        coa_conc, residuals, yerr=peak_area_std, fmt='o', markersize=10,
        capsize=6, capthick=2.5, color='coral', elinewidth=2,
        markeredgewidth=2.5, markeredgecolor='darkred', zorder=3,
    )
    ax.axhline(y=0, color='black', linestyle='-', linewidth=2, zorder=2)
    ax.axhline(
        y=3 * np.std(residuals), color='red', linestyle='--', linewidth=2,
        alpha=0.5, label='±3σ bounds',
    )
    ax.axhline(y=-3 * np.std(residuals), color='red', linestyle='--', linewidth=2, alpha=0.5)
    ...
    ax = axes[1, 0]
    ax.scatter(
        coa_conc, residuals_normalized, s=150, color='green', alpha=0.6,
        edgecolors='darkgreen', linewidth=2.5, zorder=3,
    )
    ax.axhline(y=0, color='black', linestyle='-', linewidth=2, zorder=2)
    ax.axhline(y=3, color='red', linestyle='--', linewidth=2, alpha=0.5, label='±3σ threshold')
...
v_µM = (peak_area - INTERCEPT) / SLOPE
    ...
    ax.text(
        0.05, 0.95, summary_text, transform=ax.transAxes,
    ...
    with open(calib_file, 'w') as f:
        ...
        f.write(f"  peak_area = slope × [CoA] + intercept\n\n")
...
if __name__ == '__main__':
    ...
    calibration_csv = r"C:\Users\kris\Downloads\calibration_standards..csv"
    output_dir = r"C:\Users\kris\Downloads"
    ...
    if not os.path.exists(calibration_csv):
        ...
        print(f"{'=' * 80}\n")
        print(f"Use these values in your kinetics analysis script:\n")
