...
def analyze_enzyme_kinetics_with_concentration(
    ...
    print(f"\n{'='*80}")
    print(f"ENZYME KINETICS ANALYSIS WITH PROTEIN CONCENTRATION NORMALIZATION")
    print(f"{'='*80}")
    ...
    for peak_id, group_data in df_hexcoa.groupby('peak_id'):
        ...
        try:
            ...
            print(f"  Efficiency Ranking Factor: {kcat_over_km:.0f}" if kcat_over_km > 0 else "  ⚠ Km < 0, see notes")
            print()
...
def create_comparison_plots(results_df, output_dir, enzyme_conc):
    ...
    ax = axes[0, 0]
    bars1 = ax.bar(peaks, valid_results['Km_MM'].values, color=colors, alpha=0.7, edgecolor='black', linewidth=2)
    ax.set_ylabel('Km (µM)', fontsize=12, fontweight='bold')
    ax.set_title('Substrate Affinity (Lower = Higher Affinity)', fontsize=13, fontweight='bold')
    ...
    ax = axes[0, 1]
    bars2 = ax.bar(peaks, valid_results['kcat_MM'].values, color=colors, alpha=0.7, edgecolor='black', linewidth=2)
    ax.set_ylabel(f'kcat (area/(µM·enzyme·time))', fontsize=12, fontweight='bold')
    ax.set_title('Turnover Number (Enzyme-Normalized Velocity)', fontsize=13, fontweight='bold')
    ax.grid(axis='y', alpha=0.3)
    ...
    ax = axes[1, 0]
    bars3 = ax.bar(peaks, valid_results['kcat_over_km_MM'].values, color=colors, alpha=0.7, edgecolor='black', linewidth=2)
    ax.set_ylabel('kcat/Km (dimensionless ratio)', fontsize=12, fontweight='bold')
    ax.set_title('Catalytic Efficiency (Combined Metric)', fontsize=13, fontweight='bold')
    ax.grid(axis='y', alpha=0.3)
    for i, (peak, val) in enumerate(zip(peaks, valid_results['kcat_over_km_MM'].values)):
        ax.text(i, val, f'{val:.0f}', ha='center', va='bottom', fontweight='bold', fontsize=10)
    ...
    fig.tight_layout()
    fig.savefig(f'{output_dir}/enzyme_comparison_normalized.png', dpi=300, bbox_inches='tight')
    
    print(f"✓ Comparison plot saved: enzyme_comparison_normalized.png")
...
if __name__ == '__main__':
    ...
    results = analyze_enzyme_kinetics_with_concentration(
        data_path,
        enzyme_concentration_um=enzyme_conc,
    ...
    valid = results[results['Km_MM'] > 0].sort_values('kcat_over_km_MM', ascending=False)
    
    display_cols = ['peak_id', 'Km_MM', 'Vmax_MM', 'kcat_MM', 'kcat_over_km_MM', 'R2_MM']
