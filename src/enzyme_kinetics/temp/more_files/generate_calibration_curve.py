...
def generate_calibration_curve(csv_path, output_dir='./'):
    ...
    ax = axes[0, 0]
    ax.errorbar(coa_conc, peak_area, yerr=peak_area_std, fmt='o', markersize=10,
                capsize=6, capthick=2.5, color='steelblue', elinewidth=2,
                markeredgewidth=2.5, markeredgecolor='navy', label='Measurements',
                zorder=3)
    
    coa_smooth = np.linspace(coa_conc.min() - coa_conc.max()*0.1,
    ...
    ci = 1.96 * se * np.sqrt(1/len(coa_conc) + (coa_smooth - np.mean(coa_conc))**2 / np.sum((coa_conc - np.mean(coa_conc))**2))
    ...
    # Plot 2: Residuals
    ax = axes[0, 1]
    ax.errorbar(coa_conc, residuals, yerr=peak_area_std, fmt='o', markersize=10,
    ...
    ax.axhline(y=3*np.std(residuals), color='red', linestyle='--', linewidth=2,
    ...
    ax = axes[1, 0]
    ax.scatter(coa_conc, residuals_normalized, s=150, color='green', alpha=0.6,
...
v_µM = (peak_area - INTERCEPT) / SLOPE
    ...
    fig.suptitle('HPLC Calibration Curve Analysis for CoA Quantitation',
